...
from ope import compute_is_weights_for_mdp_policy, wis_ope
...
# with open("/data/offline_rl_sepsis_simulator/data/diab_txr_mats-replication.pkl", "rb") as f:
with open("/home/offline_rl_sepsis_simulator/data/diab_txr_mats-replication.pkl", "rb") as f:
    mdict = pickle.load(f)
...
def marginalize_policy(policy, EPS):
    ...
    for i in range(n_states_abs - 2):
        ...
        # == this was the mistake made in summer ====
        ...
        new_state_vars = tuple([this_state.hr_state, this_state.sysbp_state, this_state.percoxyg_state,
                                    this_state.antibiotic_state, this_state.vaso_state, this_state.vent_state])
...
def create_data_set(num_trajectories, beh_policy, high_vol=False):
    ...
    for i in range(num_trajectories):
        for j in range(21):
            ...
            row.append(i)
            full_state_row.append(i)

            # Append state id
            row.append(states[i,j,0])
            full_state_row.append(states[i,j,0])

            # Append state variables
            this_state = State(state_idx=states[i,j,0], idx_type='full',
                               diabetic_idx=1)  # Diab a req argument, no difference

            # For this illustration, we ‘hide’
            # glucose and diabetes state in the observed trajectories
            new_state_vars = [this_state.hr_state, this_state.sysbp_state, this_state.percoxyg_state,
                                    this_state.antibiotic_state, this_state.vaso_state, this_state.vent_state]
            row += new_state_vars

            full_state_row += [this_state.hr_state, this_state.sysbp_state, this_state.percoxyg_state, this_state.glucose_state,
                               this_state.diabetic_idx, this_state.antibiotic_state, this_state.vaso_state, this_state.vent_state]
            ...
            if j == 20:
                ...
                full_state_row.append(0)
                full_state_row.append(-1)
                full_state_row += [-1] * n_actions
            else:
                row.append(rewards[i, j,0])
                row.append(actions[i, j,0])
                row += list(beh_policy[states[i, j,0], :])

                full_state_row.append(rewards[i, j,0])
                full_state_row.append(actions[i, j, 0])
                full_state_row += list(beh_policy[states[i, j, 0], :])

            row.append(difficulties[i])
            full_state_row.append(difficulties[i])

            data_set_list.append(row)
            data_set_full_state_list.append(full_state_row)

    dataset = pd.DataFrame(data_set_list, columns=["Trajectory", "State_id", "hr_state", "sysbp_state", "oxygen_state",
    ...
    full_dataset = pd.DataFrame(data_set_full_state_list, columns=["Trajectory", "State_id", "hr_state", "sysbp_state", "oxygen_state", "glucose_state",
                                                                   "diabetes_idx",
                                                                   "antibiotic_state", "vaso_state", "vent_state", "Reward", "Action_taken", "beh_p_0",
                                                                   "beh_p_1", "beh_p_2", "beh_p_3", "beh_p_4",
                                                                   "beh_p_5","beh_p_6","beh_p_7", 'difficulty'])

    return dataset, full_dataset, output
...
def smarprimer_wis(num_trajectories):
    ...
    dataset, full_dataset, output = create_data_set(num_trajectories, beh_policy)
...
if __name__ == "__main__":
    ...
    np.savez("full_state_optimal_policy", policy=policy)

    marginalized_policy = marginalize_policy(policy, EPS)

    np.savez("partial_state_optimal_policy", policy=policy)


    # MDP-Sepsis
    # POMDP-Sepsis

    # Original policy average reward with 0% random actions and std: (0.4168, 0.1596423953716556)
    ...
    # dataset, full_dataset, _ = create_data_set(200, marginalized_policy, high_vol=True)
    # dataset.to_csv("high_vol_sepsis_200_w_difficulty_05_random_extreme_v2.csv")
    # full_dataset.to_csv("high_vol_sepsis_200_w_difficulty_05_random_extreme_v2_full_states.csv")
    # print("200 was done")
    #
    # dataset, full_dataset, _ = create_data_set(1000, marginalized_policy, high_vol=True)
    # dataset.to_csv("high_vol_sepsis_1000_w_difficulty_05_random_extreme_v2.csv")
    # full_dataset.to_csv("high_vol_sepsis_1000_w_difficulty_05_random_extreme_v2_full_states.csv")
    # print("1000 was done")
    #
    # dataset, full_dataset, _ = create_data_set(5000, marginalized_policy, high_vol=True)
    # dataset.to_csv("high_vol_sepsis_5000_w_difficulty_05_random_extreme_v2.csv")
    # full_dataset.to_csv("high_vol_sepsis_5000_w_difficulty_05_random_extreme_v2_full_states.csv")
    # print("5000 was done")

    # Marginalized policy average reward with 5.0% random actions and std: (-0.0952, 0.16869304194305113)
    # Marginalized policy average reward with 0% random actions and std: (-0.0378, 0.16268505739618497)
    # Marginalized policy average reward with 5.0% random actions and std: (-0.0952, 0.16869304194305113)

    # ==== uncomment below (these are acutal lines we use to generate data)
    # dataset, full_dataset, _ = create_data_set(200, marginalized_policy, high_vol=False)
    # dataset.to_csv("marginalized_sepsis_200_w_noise_05.csv")
    # full_dataset.to_csv("marginalized_sepsis_200_w_noise_05_full_states.csv")
    # print("200 was done")
    #
    # dataset, full_dataset, _ = create_data_set(1000, marginalized_policy, high_vol=False)
    # dataset.to_csv("marginalized_sepsis_1000_w_noise_05.csv")
    # full_dataset.to_csv("marginalized_sepsis_1000_w_noise_05_full_states.csv")
    # print("1000 was done")
    #
    # dataset, full_dataset, _ = create_data_set(5000, marginalized_policy, high_vol=False)
    # dataset.to_csv("marginalized_sepsis_5000_w_noise_05.csv")
    # full_dataset.to_csv("marginalized_sepsis_5000_w_noise_05_full_states.csv")
    # print("5000 was done")
    # ======= end

    # we need to save this policy...
